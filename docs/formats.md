...
For split bags, files listed by `metadata.yaml` are processed in the listed order. If metadata is unavailable, matching files are discovered by filename.

## MCAP

MCAP files are read with the Python `mcap` library. ROSBag Doctor uses channel topic names, schema names when present, and each message's `log_time`.
