...
def _matching_rule(topic: str, rules: dict[str, TopicRule]) -> TopicRule | None:
    ...
    if exact is not None:
        return exact
    matches = [(pattern, rule) for pattern, rule in rules.items() if any(ch in pattern for ch in "*?[") and fnmatch.fnmatchcase(topic, pattern)]
    if not matches:
        return None
...
def _severity_issues_for_topic(stat: TopicStats, rule: TopicRule | None, explicit_config: bool) -> list[Issue]:
    issues: list[Issue] = []
    topic = stat.name
    ...
    if stat.duplicate_timestamps and stat.count > 1 and stat.duplicate_timestamps / (stat.count - 1) >= 0.01:
        issues.append(
            Issue(
                "warning",
    ...
    if rule.min_messages is not None and stat.count < rule.min_messages:
        issues.append(Issue("error", "too-few-messages", f"Expected at least {rule.min_messages} messages, found {stat.count}", topic=topic))
            ...
            if not lower <= measured <= upper:
                issues.append(
                    Issue(
                        "error",
                        "rate-out-of-range",
                        f"Rate {measured:.2g} Hz is outside {lower:.2f}–{upper:.2f} Hz",
                        topic=topic,
                        details={"measured_hz": measured, "expected_hz": rule.rate_hz, "tolerance": rule.rate_tolerance},
                    )
                )
    if rule.max_gap_ms is not None and stat.max_gap_ms is not None and stat.max_gap_ms > rule.max_gap_ms:
        issues.append(Issue("error", "gap-too-large", f"Maximum gap {stat.max_gap_ms:.1f} ms exceeds {rule.max_gap_ms:.1f} ms", topic=topic))
    if rule.max_jitter_ms is not None and stat.p95_jitter_ms is not None and stat.p95_jitter_ms > rule.max_jitter_ms:
        issues.append(Issue("error", "jitter-too-high", f"p95 jitter {stat.p95_jitter_ms:.2f} ms exceeds {rule.max_jitter_ms:.2f} ms", topic=topic))
    if rule.min_coverage is not None and stat.coverage is not None and stat.coverage < rule.min_coverage:
        issues.append(Issue("error", "coverage-too-low", f"Coverage {stat.coverage:.1%} is below {rule.min_coverage:.1%}", topic=topic))
    if rule.max_start_delay_ms is not None and stat.start_delay_ms is not None and stat.start_delay_ms > rule.max_start_delay_ms:
        issues.append(Issue("error", "starts-too-late", f"Topic starts {stat.start_delay_ms:.1f} ms after bag start", topic=topic))
    if rule.max_end_early_ms is not None and stat.end_early_ms is not None and stat.end_early_ms > rule.max_end_early_ms:
        issues.append(Issue("error", "ends-too-early", f"Topic ends {stat.end_early_ms:.1f} ms before bag end", topic=topic))
    return issues
...
def run_checks(
    ...
    *,
    ...
    if config.bag.min_duration_s is not None and duration_s < config.bag.min_duration_s:
        issues.append(Issue("error", "bag-too-short", f"Bag duration {duration_s:.2f} s is below {config.bag.min_duration_s:.2f} s"))
    if config.bag.max_duration_s is not None and duration_s > config.bag.max_duration_s:
        issues.append(Issue("error", "bag-too-long", f"Bag duration {duration_s:.2f} s exceeds {config.bag.max_duration_s:.2f} s"))
    if config.bag.min_messages is not None and bag.total_messages < config.bag.min_messages:
        issues.append(Issue("error", "bag-too-few-messages", f"Bag contains {bag.total_messages} messages; expected at least {config.bag.min_messages}"))
    ...
    for pattern, rule in config.topics.items():
        ...
        if not matched:
            issues.append(Issue("error", "required-topic-missing", f"Required topic or pattern is missing: {pattern}", topic=pattern))

    for stat in stats:
        if _ignored(stat.name, config.ignore):
            continue
        rule = _matching_rule(stat.name, config.topics)
        issues.extend(_severity_issues_for_topic(stat, rule, explicit_config))
    ...
    for rule in config.sync:
        ...
        if rule.max_p95_offset_ms is not None and p95 is not None and p95 > rule.max_p95_offset_ms:
            issues.append(Issue("error", "sync-p95-too-high", f"Sync '{rule.name}' p95 offset {p95:.2f} ms exceeds {rule.max_p95_offset_ms:.2f} ms", details={"sync": rule.name, "p95_offset_ms": p95}))
        if rule.max_offset_ms is not None and max_offset is not None and max_offset > rule.max_offset_ms:
            issues.append(Issue("error", "sync-max-too-high", f"Sync '{rule.name}' max offset {max_offset:.2f} ms exceeds {rule.max_offset_ms:.2f} ms", details={"sync": rule.name, "max_offset_ms": max_offset}))
